...
def CrossCorrelationByGene(ribowigfilename,netwigfilename,outcorrfilename):
  ...
  for line in CDSlist:
      if line[3]=='+' and (line[2]-line[1]) > 50:
      ...
      elif line[3]=='-' and (line[2]-line[1]) > 50:
        genename=line[0]
        #print line[0]
        ribogene=[]
        netgene=[]
        ...
        if numpy.mean(ribogene)>0 and numpy.mean(netgene) >0:
          netgene = (netgene - numpy.mean(netgene)) / (numpy.std(netgene) * len(netgene))
          ribogene = (ribogene - numpy.mean(ribogene)) / numpy.std(ribogene)
          ...
          correlationgene=[]
          for r in range(50):
            correlation = numpy.correlate(netgene,ribogene)
            netgene.pop(-1)
            #x = ribogene.pop(-1)
            #ribogene.insert(0,0)
            correlationgene.append(correlation[0])
...
def main():
  CrossCorrelationByGene(sys.argv[1],sys.argv[2],sys.argv[3])
